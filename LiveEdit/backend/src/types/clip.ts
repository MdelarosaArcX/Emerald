...
/**
 * A single applied effect on a clip. Kept intentionally generic
 * so the FFmpeg/render pipeline can be implemented later without
...
export interface Clip {
  ...
  effects: ClipEffect[];
  type: ClipType;
  /** Optional transform/playback properties surfaced in the Inspector panel */
  opacity?: number;
  rotation?: number;
  scale?: number;
