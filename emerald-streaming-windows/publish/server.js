...
app.post("/api/obs-recording/start", (request, response) => {
  try {
    response.json(obsRecorder.start(request.body || {}));
  } catch (error) {
...
app.post("/api/obs-preview/start", (request, response) => {
  try {
    response.json(obsPreview.start(request.body || {}));
  } catch (error) {
    response.status(400).json({ message: error.message });
  }
});
